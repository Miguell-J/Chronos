...
argsp = argsubparsers.add_parser("branch", help="Create, list or delete branches")
argsp.add_argument("name", nargs="?", help="Name of the branch to create or switch to")
argsp.add_argument("-d", "--delete", action="store_true", help="Delete the specified branch")
...
def branch_create(repo, name):
    sha = ref_resolve(repo, "HEAD")
    if not sha:
        raise Exception("Cannot create branch: HEAD is invalid or repository has no commits.")

    path = repo_file(repo, "refs", "heads", name)
    if os.path.exists(path):
...
#and now we list then
def branch_list(repo):
    heads_dir = repo_dir(repo, "refs", "heads")
    current = branch_get_active(repo)
    for branch in sorted(os.listdir(heads_dir)):
...
def branch_delete(repo, name):
    current = branch_get_active(repo)
    if current == name:
        raise Exception("Cannot delete the currently checked-out branch.")
